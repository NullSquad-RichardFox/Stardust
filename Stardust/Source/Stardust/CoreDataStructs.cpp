...
UStructDataLibrary::UStructDataLibrary()
{
	...
	WienerProcessDataTable = LoadDataTable("DataTable'/Game/DataTables/DT_WienerProcessData.DT_WienerProcessData'");
}
...
const FWienerProcessData* UStructDataLibrary::GetWienerProcess(EResourceType ResourceType)
{
	if (!WienerProcessDataTable) return nullptr;


	if (UEnum* Enum = FindObject<UEnum>(ANY_PACKAGE, TEXT("EResourceType"), true))
	{
		FString ContextString("UStructDataLibrary");
		return WienerProcessDataTable->FindRow<FWienerProcessData>(*Enum->GetNameStringByIndex((int32)ResourceType), ContextString);
	}

...
void AGameFramework::BeginPlay()
{
	...
	// Spawns player corporation class
	GetWorld()->SpawnActor<APlayerCorporation>();

	// Creates playable map
	GenerateMap(10);

	// Creates pathfinder class to make trade routes
	Pathfinder = UPathfinder(this);

	// Sets up resource price processes
	SetUpWienerProcesses();
}
...
void AGameFramework::StockMarket()
{
	...
	/*for (const auto& [ResType, Process] : ResourcePriceProcesses)
	{
		...
		if (Process.GetPrice() <= 0)
			LOG_ERROR("Resource price is negative - Price: [%f], Resource: [%s]", Process.GetPrice(), *ResourceEnum->GetNameStringByValue((int32)ResType))

		const FWienerProcessData* Data = UStructDataLibrary::GetWienerProcess(ResType);

		if (Process.GetPrice() <= Data->Price * 0.2)
			LOG("Resource is at low price - Price: [%f], Resource: [%s]", Process.GetPrice(), *ResourceEnum->GetNameStringByValue((int32)ResType))

		if (Process.GetPrice() >= Data->Price * 1.35)
			LOG("Resource is at high price - Price: [%f], Resource: [%s]", Process.GetPrice(), *ResourceEnum->GetNameStringByValue((int32)ResType))

		for (const auto& [CorrRes, Rho] : Data->CorrelatedProcesses)
		{
...
void AGameFramework::SetUpWienerProcesses()
{
	...
	for (int32 i = 0; i < ResourceEnum->NumEnums(); i++)
	{
		EResourceType ResourceType = (EResourceType)i;
		const FWienerProcessData* Data = UStructDataLibrary::GetWienerProcess(ResourceType);
